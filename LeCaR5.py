...
class LeCaR5(page_replacement_algorithm):
    ...
    def __init__(self, cache_size, learning_rate=0, initial_weight=0.5, discount_rate=1, visualize=0):
        ...
        self.SampleChangeInHR_SIZE = 100
...
#         return np.array([self.pollution_dat_x,self.pollution_dat_y ]).T
    ...
    def updateLearningRates(self,seq_len):
        if self.time % (seq_len) == 0:
            ...
            self.NewHR = round(self.CacheHit/ float(seq_len),6)
            ...
            self.SampleChangeInHR.put(self.NewHR)
            self.SampleCIR += self.NewHR 

            
            
            delta_LR = round(self.NewLR,5) - round(self.PreviousLR,5)
            if (delta_LR) >= 0.00001 :
                    delta = round(self.NewChangeInHR / delta_LR, 6)
           
            # if abs(self.NewChangeInHR) <=0.005:
            #         delta_1 = 0.1  
            if delta>0:
                ...
                # if self.NewChangeInHR <=0.05:
                #     self.NewChangeInHR =0.1
            ...
            elif self.NewChangeInHR <= 0 and delta == 0:

                # if  (self.SampleCIR / float(self.SampleChangeInHR_SIZE)) <0.001:
                #     self.learning_rate = 0.45
                ...
                # else:
                    if self.learning_rate >= 0.999:
                        ...
                        val= np.random.rand()
                        if  val <=0.5:
                            self.learning_rate = min(self.learning_rate + (float(self.learning_rate)/5.0)  ,1)
                        # elif val>0.34 and val<0.68: 
                        else:
                            self.learning_rate =max( self.learning_rate - (float(self.learning_rate)/5.0)  , 0.001)
                        # print("LR equal and LR not extreme", delta_LR, self.NewChangeInHR , self.learning_rate)
                        # print(self.learning_rate)
                # else:
                #     print("Inside equal and positive performnace update", delta_1, self.NewChangeInHR ,self.learning_rate)
