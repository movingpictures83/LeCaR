...
#     def __init__(self, N, visualization = True):
    ...
    def __init__(self, cache_size, learning_rate=0, initial_weight=0.5, discount_rate=1, visualize=0):
        ...
        self.LR = np.array([self.learning_rate,self.learning_rate], dtype=np.float32)
        ...
        self.W_1 = np.array([.5,.5], dtype=np.float32)
        self.W_2 = np.array([.5,.5], dtype=np.float32)
        self.qUsed = {}
        self.eTime = {}
...
#         q = self.getQ()
    ...
    def request(self,page) :
        ...
        seq_len = self.N /2
        if len(self.CacheHitList)> seq_len:
...
#                 reward[1] = -1 ## punish
            ...
            if self.time % (seq_len) == 0:
                ...
                delta = 0.1
                # delta =  np.exp( self.NewChangeInHR  ) 
                delta = (self.NewHR -self.PreviousHR)

                average_hitrate = self.CacheHit / self.time
                # if self.NewHR < average_hitrate  and self.learning_rate <= 0.1:
                #     self.learning_rate = 0.45
                # print("change in HR", delta)
                # if(self.NewChangeInHR > self.PreviousChangeInHR):

                # self.learning_rate += delta
                # if(self.NewHR > self.PreviousHR):
                #     # if  self.learning_rate -delta >= 0.01  :
                #         self.learning_rate -=  delta
                #         if  self.learning_rate <= 0.01  :
                #             self.learning_rate = 0.01
                    
                self.LR += np.array([delta,delta])/ (self.W_1 - self.W)
                

                # print(self.learning_rate)
                # if(self.NewHR < self.PreviousHR):
                #     # if self.learning_rate +delta  <= 1  :
                        # self.learning_rate += delta 
                #         if self.learning_rate >= 1  :
                #             self.learning_rate = 0.99
                    
                # if  self.learning_rate <= 0.1 and self.NewHR < average_hitrate  :
                #         self.learning_rate = 0.45
                # elif self.learning_rate >= 0.9 and self.NewHR > average_hitrate :
                #         self.learning_rate =0.45

                # if  self.NewHR <= self.PreviousHR and self.NewHR <= average_hitrate :
                #         self.learning_rate += delta
                # if self.NewHR > self.PreviousHR and self.NewHR > average_hitrate :
                #         self.learning_rate -=  delta
            ...
            if pageevict is not None  :
                self.W_1 = self.W
                self.W = self.W * np.exp(self.LR * reward / 2)
...
#                 self.qUsed[cacheevict] = self.getQ()[poly]
        ...
        self.learning_rates.append(self.LR[0])
        return page_fault
